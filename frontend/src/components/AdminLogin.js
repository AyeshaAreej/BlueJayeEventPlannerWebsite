...
function AdminLogin() {
  ...
  {({ touched, errors,handleSubmit, handleBlur, values,handleChange }) =>( 
          ...
          <form onSubmit={handleSubmit} >  
           <label htmlFor="emil1" style={{marginLeft:'15%'}}>Email</label>
           <input type="text"  id="emil1"   name='email' placeholder="Enter your email..."  onBlur={handleBlur} onChange={handleChange} value={values.email} />
            {touched.email && errors.email ? <div className='error' style={{  fontSize: 15, color: 'red',marginLeft:"15%", marginBottom:"2%"}}> {errors.email}</div> : null}
                      
          <label htmlFor="pwd1"  style={{marginLeft:'15%'}}>Password</label>
          <input type="password" id="pwd1" name="password" placeholder="Enter password"  autoComplete="false"
